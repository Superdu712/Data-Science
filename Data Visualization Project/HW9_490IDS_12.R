...
#Research LifeCycles of plot creation.
#In the first stage of the cycle, we need to have a data management plan of what data I will be using,
#the geographic, demographic factors that restrict the data to a certain amount, and credible
#sources to prepare the data. Then collecting the data from National Weather Service Forecast Office,
#and we choose two locations: San Francisco and Los Angeles, and use the monthly temperature data for
#a certain year 2011. Once we have the data, we need to process the data, and create variables, and
#functions for plot creation. The steps are: makeplotregion, drawTempregion, monthPrecip, addgridlines.
#When we have the simple basic plot, the next step is to improve the quality of the plot by adjusting
#font size, text location, font style, etc. Then we will finally plot the graph, and then we
#will save the plot for further research use.