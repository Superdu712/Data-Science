...
finalPlot = function(temp, precip, temp1, precip1){
  ...
  pdf("EC_Plot2", width = 27, height = 12)
  layout(matrix(c(1,3,2,4), nrow = 2, ncol = 2, byrow = TRUE), height = c(2,1,2,1))
  ...
  text(tiedHighday[which(tiedHigh == max(tiedHigh))] + 1, max(tiedHigh) + 10, 
       labels = paste("TIED RECORD HIGH: ", max(tiedHigh)), cex = 0.75)
  ...
  ##which(recordLow == sfoWeather$Low) returns integer(0), so there is no tiedLow and tiedLowday.
  
  ### Add the titles 
  
  title(main = list("San Francisco's Weather in 2015", cex = 2.1), adj=0)
  ...
  makePlotRegion(xlim = c(1, 365), ylim = c(0, 5), bgcolor = "#ededea",
                 ylabels = seq(0, 5, 1), margins = c(4,3.5,5,3.5), cityName = "", xtop = FALSE)
  ...
  mtext(side = 3, line = 2, text = paste("Cumulative monthly precipitation in inches compared with normal monthly precipitation. Total precipitation in 2015 was", sum(temp$Precip), "inches."))
  text(x=3, y=4.9, labels = "NORMAL", cex = 0.8, font = 3)
  text(x=13, y=0.3, labels = "ACTUAL", cex = 0.8)

  ###############################################################################################
  # Here are some vectors that you might find handy
  
  normPrecip = as.numeric(as.character(precip1$normal))
  ...
  temp1$Precip[is.na(temp1$Precip)] = 0
  recordPrecip1 = temp1$RecordPrecip
  recordHigh1 = temp1$RecordHigh
  recordLow1 = temp1$RecordLow
  normalHigh1 = temp1$NormalHigh
  normalLow1 = temp1$NormalLow
  tiedHigh1 = temp1$High[which(recordHigh1 == temp1$High)]
  tiedLow1 = temp1$Low[which(recordLow1 == temp1$Low)]
  tiedHighday1 = which(recordHigh1 == temp1$High)
  tiedLowday1 = which(recordLow1 == temp1$Low)
  ...
  drawTempRegion(c(1:365), recordHigh1, recordLow1, col = "#e8d8be")
  drawTempRegion(c(1:365), normalHigh1, normalLow1, col = "#a59789")
  drawTempRegion(c(1:365), temp1$High, temp1$Low, col = "#681746")
  ...
  text(tiedHighday1[which(tiedHigh1 == max(tiedHigh1))] + 1, max(tiedHigh1) + 10, 
       labels = paste("TIED RECORD HIGH: ", max(tiedHigh1)), cex = 0.75)
  segments(tiedHighday1[which(tiedHigh1 == max(tiedHigh1))], max(tiedHigh1), y1 = max(tiedHigh1) + 10)
  
  text(tiedLowday1[which(tiedLow1 == min(tiedLow1))] + 1, min(tiedLow1) - 10, 
       labels = paste("TIED RECORD LOW:", min(tiedLow1)), cex = 0.75)
  segments(tiedLowday1[which(tiedLow1 == min(tiedLow1))], min(tiedLow1), y1 = min(tiedLow1) - 10)
  
  ### Add the titles 
  
  title(main = list("Los Angeles's Weather in 2015", cex = 2.1), adj=0)
  text(5, 108, labels = "Temperature", cex = 1.5, font = 2)
  text(5, 102.5, labels = "Bars represent range between the", cex = 1.2)
  text(5, 99, labels = "daily high and low.", cex = 1.2)
  segments(190, 23, y1 = 38, col = "#e8d8be", lwd = 7)
  ...
  sapply(1:12, function(m){
    monthPrecip(day = cumDays[m]:(cumDays[m+1]-1),
                dailyprecip = temp1$Precip[cumDays[m]:(cumDays[m+1]-1)],
                normal = normPrecip[m])
  })
  ...
  title(main = list("Precipitation", cex=1.5, font=2))
  mtext(side = 3, line = 2, text = paste("Cumulative monthly precipitation in inches compared with normal monthly precipitation. Total precipitation in 2015 was", sum(temp1$Precip), "inches."))
  text(x=3, y=3.4, labels = "NORMAL", cex = 0.8, font = 3)
  text(x=11, y=1.5, labels = "ACTUAL", cex = 0.8)  
  
  ### Close the pdf device dev.off()
  dev.off()
}

### Call: finalPlot(temp = sfoWeather, precip = sfoMonthlyPrecip, temp1 = laxWeather, precip1 = laxMonthlyPrecip)
finalPlot(temp = sfoWeather, precip = sfoMonthlyPrecip, temp1 = laxWeather, precip1 = laxMonthlyPrecip)