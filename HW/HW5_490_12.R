...
# add legend to the top right corner, name it respectively, use the same line colors, line types, and line width, make the legend no border.

# Do the same improvements for the rest two graphs.
hist(b2,freq=F, ylim=c(0,0.15), xlab="rbinom(m=200, n=30, p=0.5)", main="Probability Density",col="gray99")
curve(dnorm(x,mean=mean(b2), sd=sd(b2)),add=T, lwd=2, col="midnightblue")
lines(density(b2), lty=3, lwd=2.5, col="violetred1")
legend("topright", c("Normal Curve","Density Curve"), cex=0.8, col=c("midnightblue","violetred1"), lty=c(1,3), lwd=c(2,2.5), bty="n")

hist(b3,freq=F, ylim=c(0,0.12), xlab="rbinom(m=200, n=50, p=0.5)", main="Probability Density", col="gray99")
...
#
rm(list = ls()) # Before loading the data, remove all the values in workspace.
...
### Your answer here
#[1] "county"  "city"    "zip"     "street"  "price"   "br"     
#[7] "lsqft"   "bsqft"   "year"    "date"    "long"    "lat"    
#[13] "quality" "match"   "wk"     
